...
public class SbccPrinter {
  ...
  if (sbccVerificationResult.getAccept().equals(ACCEPT)) {
   ...
   for (final SbccRule sbccRule : sbccVerificationResult.getRules()) {
    if (sbccRule.getMessage().isPresent()) {
     sbccRenderer.append(sb, "  " + sbccRule.getMessage().get() + ": " + sbccRule.getRegexp() + NL);
    }
   }
  }
 }
 ...
 private void printCommit(final SbccChangeSet sbccChangeSet, StringBuilder sb) {
  sbccRenderer.append(sb, NL);
  sbccRenderer.append(sb, NL);
  sbccRenderer.append(sb, sbccChangeSet.getId() + " " + sbccChangeSet.getAuthor().getName() + " <"
    + sbccChangeSet.getAuthor().getEmailAddress() + ">" + NL);
  sbccRenderer.append(sb, ">>> " + sbccChangeSet.getMessage() + NL);
 }

 private void printEmailVerification(final SbccRefChangeVerificationResult refChangeVerificationResult,
   final SbccChangeSet sbccChangeSet, StringBuilder sb) {
  if (!refChangeVerificationResult.getSbccChangeSets().get(sbccChangeSet).getEmailAuthorResult()) {
   sbccRenderer.append(sb, NL);
   sbccRenderer.append(sb, "- Bitbucket: '" + matchedEmail("${" + SbccRenderer.SBCCVariable.BITBUCKET_EMAIL + "}")
     + "' != Commit: '" + sbccChangeSet.getAuthor().getEmailAddress() + "'" + NL);
  ...
  if (!refChangeVerificationResult.getSbccChangeSets().get(sbccChangeSet).getEmailCommitterResult()) {
   sbccRenderer.append(sb, NL);
   sbccRenderer.append(sb, "- Bitbucket: '" + matchedEmail("${" + SbccRenderer.SBCCVariable.BITBUCKET_EMAIL + "}")
     + "' != Commit: '" + sbccChangeSet.getCommitter().getEmailAddress() + "'" + NL);
   if (settings.getRequireMatchingAuthorEmailMessage().isPresent()) {
    sbccRenderer.append(sb, "  " + settings.getRequireMatchingAuthorEmailMessage().get() + NL);
   }
  }
  if (!refChangeVerificationResult.getSbccChangeSets().get(sbccChangeSet)
   ...
   sbccRenderer.append(sb, NL);
   sbccRenderer.append(sb, "- Commit: '" + sbccChangeSet.getAuthor().getEmailAddress() + "'" + NL);
   if (settings.getRequireMatchingAuthorEmailMessage().isPresent()) {
    sbccRenderer.append(sb, "  " + settings.getRequireMatchingAuthorEmailMessage().get() + NL);
   }
  }
 ...
 private void printNameVerification(final SbccRefChangeVerificationResult refChangeVerificationResult,
   final SbccChangeSet sbccChangeSet, StringBuilder sb) {
  if (!refChangeVerificationResult.getSbccChangeSets().get(sbccChangeSet).getNameAuthorResult()) {
   sbccRenderer.append(sb, NL);
   sbccRenderer.append(sb, "- Bitbucket: '${" + SbccRenderer.SBCCVariable.BITBUCKET_NAME + "}' != Commit: '"
     + sbccChangeSet.getAuthor().getName() + "'" + NL);
  ...
  if (!refChangeVerificationResult.getSbccChangeSets().get(sbccChangeSet).getNameCommitterResult()) {
   sbccRenderer.append(sb, NL);
   sbccRenderer.append(sb, "- Bitbucket: '${" + SbccRenderer.SBCCVariable.BITBUCKET_NAME + "}' != Commit: '"
     + sbccChangeSet.getCommitter().getName() + "'" + NL);
   if (settings.getRequireMatchingAuthorNameMessage().isPresent()) {
    sbccRenderer.append(sb, "  " + settings.getRequireMatchingAuthorNameMessage().get() + NL);
   }
  }
 ...
 private void printMaximumSizeExceeded(Map<String, Long> map, StringBuilder sb) {
  for (String file : map.keySet()) {
   ...
   sbccRenderer.append(sb, NL);
   sbccRenderer.append(sb, "- " + file + " " + sizeKb + "kb > " + settings.getCommitSizeKb() + "kb" + NL);
 ...
 private void printJqlVerification(SbccRefChangeVerificationResult refChangeVerificationResult,
   SbccChangeSet sbccChangeSet, StringBuilder sb) {
  for (String query : refChangeVerificationResult.getSbccChangeSets().get(sbccChangeSet).getFailingJqls()) {
   sbccRenderer.append(sb, NL);
 ...
 public String printVerificationResults(SbccVerificationResult verificationResult) {
  ...
  for (final SbccRefChangeVerificationResult refChangeVerificationResult : verificationResult.getRefChanges()) {
   ...
   for (final SbccChangeSet sbccChangeSet : refChangeVerificationResult.getSbccChangeSets().keySet()) {
    ...
    for (final SbccGroup sbccVerificationResult : changeSetVerificationResult.getGroupsResult().keySet()) {
