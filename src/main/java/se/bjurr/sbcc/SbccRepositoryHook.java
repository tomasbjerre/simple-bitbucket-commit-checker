...
public class SbccRepositoryHook {
  ...
  public Optional<Settings> findSettings(final Repository repository) {
    try {
      ...
      RepositoryHook hook =
          securityService
              .withPermission(REPO_ADMIN, "Configuration")
              .call(
                  new UncheckedOperation<RepositoryHook>() {
                    @Override
                    public RepositoryHook perform() {
                      return repositoryHookService.getByKey(repository, HOOK_SETTINGS_KEY);
                    }
                  });
      ...
      SbccSettings abccSettings = SbccSettings.sscSettings(settings);
