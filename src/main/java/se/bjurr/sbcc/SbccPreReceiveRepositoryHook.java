...
public class SbccPreReceiveRepositoryHook implements PreReceiveRepositoryHook {
 ...
 public boolean onReceive(RepositoryHookContext repositoryHookContext, Collection<RefChange> refChanges,
  ...
  try {
   ...
   if (new UserValidator(settings, this.bitbucketAuthenticationContext.getCurrentUser()).shouldIgnoreChecksForUser()) {
    return TRUE;
   }

   final SbccVerificationResult refChangeVerificationResults = new RefChangeValidator(
     repositoryHookContext.getRepository(), repositoryHookContext.getRepository(), settings, this.changesetsService,
     this.bitbucketAuthenticationContext, sbccRenderer, this.applicationLinkService, this.sbccUserAdminService)
