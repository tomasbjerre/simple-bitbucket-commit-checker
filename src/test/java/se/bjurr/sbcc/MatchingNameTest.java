...
public class MatchingNameTest {
 ...
 public void testCommitMustMatchNameInBitbucketAndOnlyOneOfTwoCommitsIsOk() throws IOException {
  refChangeBuilder()
    ...
    .withChangeSet(
      changeSetBuilder().withId("2").withCommitter(new SbccPerson("Tompa B", "tomas.bjerre@two.site"))
    ...
    .withSetting(SETTING_REQUIRE_MATCHING_AUTHOR_NAME_MESSAGE, "Name in Bitbucket not same as in commit")
    .build()
    .run()
 ...
 public void testCommitMustMatchNameInBitbucketAndOnlyOneOfTwoCommitsIsOkReverseOrder() throws IOException {
  refChangeBuilder()
    .withChangeSet(
      changeSetBuilder().withId("1").withCommitter(new SbccPerson("Tomas B", "committer@one.site"))
        .withAuthor(new SbccPerson("Tomas Author", "author@one.site")).withMessage(COMMIT_MESSAGE_JIRA).build())
    .withChangeSet(
 ...
 public void testCommitIsAcceptedWhenNameMustMatchNameInBitbucketAndTwoOfTwoCommitsHaveDifferentEmail()
  ...
  refChangeBuilder()
    ...
    .withBitbucketDisplayName("Tomas Committer").withSetting(SETTING_REQUIRE_MATCHING_COMMITTER_NAME, TRUE)
 ...
 public void testCommitAuthorMustMatchNameInBitbucketAndOnlyOneOfTwoCommitsIsOk() throws IOException {
  refChangeBuilder()
    .withChangeSet(
      changeSetBuilder().withId("1").withCommitter(new SbccPerson("Tomas Committer", "committer@one.site"))
        .withAuthor(new SbccPerson("Tomas Diff", "author@one.site")).withMessage(COMMIT_MESSAGE_JIRA).build())
    .withChangeSet(
      changeSetBuilder().withId("2").withCommitter(new SbccPerson("Tomas Committer", "tomas.bjerre@two.site"))
        .withAuthor(new SbccPerson("Tomas Author", "author@one.site")).withMessage(COMMIT_MESSAGE_JIRA).build())
    .withBitbucketEmail("committer@one.site")
  ...
  refChangeBuilder()
    ...
    .withChangeSet(
      changeSetBuilder().withId("2").withCommitter(new SbccPerson("Tomas Committer 2", "tomas.bjerre@two.site"))
        .withAuthor(new SbccPerson("Tomas Author", "author@one.site")).withMessage(COMMIT_MESSAGE_JIRA).build())
    .withBitbucketEmail("committer@one.site").withBitbucketDisplayName("Tomas Author")
    .withSetting(SETTING_REQUIRE_MATCHING_AUTHOR_NAME, TRUE)
    .withSetting(SETTING_REQUIRE_MATCHING_AUTHOR_NAME_MESSAGE, "Name in Bitbucket not same as in commit").build().run()
    .hasNoOutput().wasAccepted();
 }
 ...
 public void testThatAuthorNameCanBeRejectedIfNotInBitbucket() throws IOException {
  refChangeBuilder()
    ...
    .withSetting(SETTING_REQUIRE_MATCHING_AUTHOR_NAME_MESSAGE, "Name not available in Bitbucket")
 ...
 public void testThatAuthorNameCanBeAcceptedIfInBitbucket() throws IOException {
  refChangeBuilder()
    ...
    .withSetting(SETTING_REQUIRE_MATCHING_AUTHOR_NAME_BITBUCKET, TRUE)
    .withSetting(SETTING_REQUIRE_MATCHING_AUTHOR_NAME_MESSAGE, "Name not available in Bitbucket")
    .withUserInBitbucket("Tomas Author", "user.name", "user@email").build().run().hasNoOutput().wasAccepted();
 }
}
