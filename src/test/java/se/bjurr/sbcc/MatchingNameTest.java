...
public class MatchingNameTest {
 ...
 public void testCommitIsAcceptedWhenNameMustMatchNameInBitbucketAndTwoOfTwoCommitsHaveDifferentEmail()
  ...
  refChangeBuilder()
    ...
    .withBitbucketEmail("committer@one.site").withBitbucketDisplayName("Tomas Committer")
 ...
 public void testCommitMustMatchNameInBitbucketAndOnlyOneOfTwoCommitsIsOk() throws IOException {
  refChangeBuilder()
    .withChangeSet(changeSetBuilder().withId("1").withCommitter(new SbccPerson("Tomas Committer", "committer@one.site"))
      .withAuthor(new SbccPerson("Tomas Author", "author@one.site")).withMessage(COMMIT_MESSAGE_JIRA).build())
    .withChangeSet(changeSetBuilder().withId("2").withCommitter(new SbccPerson("Tompa B", "tomas.bjerre@two.site"))
      .withMessage(COMMIT_MESSAGE_JIRA).build())
    .withBitbucketEmail("committer@one.site").withBitbucketDisplayName("Tompa Committer")
    .withSetting(SETTING_REQUIRE_MATCHING_COMMITTER_NAME, TRUE)
    .withSetting(SETTING_REQUIRE_MATCHING_AUTHOR_NAME_MESSAGE, "Name in Bitbucket not same as in commit").build().run()
    .hasTrimmedFlatOutput(
 ...
 public void testCommitMustMatchNameInBitbucketAndOnlyOneOfTwoCommitsIsOkReverseOrder() throws IOException {
  refChangeBuilder()
    .withChangeSet(changeSetBuilder().withId("1").withCommitter(new SbccPerson("Tomas B", "committer@one.site"))
      .withAuthor(new SbccPerson("Tomas Author", "author@one.site")).withMessage(COMMIT_MESSAGE_JIRA).build())
    .withChangeSet(
      changeSetBuilder().withId("2").withCommitter(new SbccPerson("Tomas Committer", "tomas.bjerre@two.site"))
        .withMessage(COMMIT_MESSAGE_JIRA).build())
 ...
 public void testThatAuthorNameCanBeRejectedIfNotInBitbucket() throws IOException {
  refChangeBuilder()
    ...
    .withSetting(SETTING_REQUIRE_MATCHING_AUTHOR_NAME_BITBUCKET, TRUE)
    .withSetting(SETTING_REQUIRE_MATCHING_AUTHOR_NAME_MESSAGE, "Name not available in Bitbucket")
    .withUserInBitbucket("Display Name", "user.name", "user@email").build().run()
    .hasTrimmedFlatOutput(
