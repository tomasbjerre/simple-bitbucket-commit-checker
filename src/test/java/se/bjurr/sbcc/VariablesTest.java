...
public class VariablesTest {
 ...
 @Test
 public void testBitbucketAuthorEmailVariableCanBeUsedInEmailRejectionMessage() throws IOException {
  refChangeBuilder()
    .withChangeSet(changeSetBuilder().withId("1").withCommitter(new SbccPerson("Commit Name", "commit@mail"))
      .withMessage(COMMIT_MESSAGE_JIRA).withAuthor(new SbccPerson("Commit Name", "author@mail")).build())
    .withBitbucketEmail("bitbucket@mail").withBitbucketDisplayName("Bitbucket Name")
    .withSetting(SETTING_REQUIRE_MATCHING_AUTHOR_EMAIL, TRUE)
    .withSetting(SETTING_REQUIRE_MATCHING_AUTHOR_EMAIL_MESSAGE,
      "Bitbucket says your email is ${" + SbccRenderer.SBCCVariable.BITBUCKET_EMAIL
        + "}, set it using: git config --global user.email ${" + SbccRenderer.SBCCVariable.BITBUCKET_EMAIL + "}")
    .build().run()
 ...
 public void testBitbucketAuthorNameVariableCanBeUsedInNameRejectionMessage() throws IOException {
  refChangeBuilder()
    ...
    .withSetting(SETTING_REQUIRE_MATCHING_AUTHOR_NAME_MESSAGE,
      "Bitbucket says your name is ${" + SbccRenderer.SBCCVariable.BITBUCKET_NAME
        + "}, set it using: git config --global user.name \"${" + SbccRenderer.SBCCVariable.BITBUCKET_NAME + "}\"")
    .build().run()
    .hasTrimmedFlatOutput(
