...
public class DryRunTest {
 ...
 public void testThatDryRunAcceptsFaultyCommits() throws IOException {
  refChangeBuilder().withSetting(SETTING_DRY_RUN, TRUE).withGroupAcceptingAtLeastOneJira()
    .withChangeSet( //
      changeSetBuilder() //
        .withId("1") //
        .withMessage(COMMIT_MESSAGE_NO_ISSUE) //
        .build() //
    ).build().run().hasTrimmedFlatOutput("refs/heads/master e2bc4ed003 -> af35d5c1a4   1 Tomas <my@email.com> >>> fixing stuff  - You need to specity an issue   JIRA: ((?<!([A-Z]{1,10})-?)[A-Z]+-\\d+)").wasAccepted();
 }
  ...
  refChangeBuilder().withSetting(SETTING_DRY_RUN, TRUE).withSetting(SETTING_DRY_RUN_MESSAGE, "In dry run mode!")
    ...
    .withChangeSet( //
      changeSetBuilder() //
        ...
        .withMessage(COMMIT_MESSAGE_NO_ISSUE) //
        .build() //
    ).build().run().hasTrimmedFlatOutput("refs/heads/master e2bc4ed003 -> af35d5c1a4   1 Tomas <my@email.com> >>> fixing stuff  - You need to specity an issue   JIRA: ((?<!([A-Z]{1,10})-?)[A-Z]+-\\d+)  In dry run mode!").wasAccepted();
 }
