...
public class GroupOneRuleOneMatchingNoneAcceptTest {
 ...
 public void testThatItCanAcceptMultipleCommits() throws IOException {
  refChangeBuilder()
    ...
    .run()
    .hasTrimmedFlatOutput(
      "refs/heads/master e2bc4ed003 -> af35d5c1a4   2 Tomas <my@email.com> >>> SB-5678 fixing stuff INC123  - Do not specify issues   JIRA: ((?<!([A-Z]{1,10})-?)[A-Z]+-\\d+)   Incident, INC: INC")
    .wasRejected();
 }
 ...
 public void testThatItCanRejectSomeOfMultipleCommits() throws IOException {
  refChangeBuilder()
    ...
    .run()
    .hasTrimmedFlatOutput(
      "refs/heads/master e2bc4ed003 -> af35d5c1a4   1 Tomas <my@email.com> >>> fixing stuff INC123  - Do not specify issues   JIRA: ((?<!([A-Z]{1,10})-?)[A-Z]+-\\d+)   Incident, INC: INC   2 Tomas <my@email.com> >>> SB-5678 fixing stuff INC123  - Do not specify issues   JIRA: ((?<!([A-Z]{1,10})-?)[A-Z]+-\\d+)   Incident, INC: INC")
    .wasRejected();
 }
}
